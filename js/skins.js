...
document.addEventListener('click', function (e) {
    ...
    switch (e.target.getAttribute('data-action')) {
        ...
        case 'weapon_change':
            ...
            if (!e.target.getAttribute('data-paint')) {
        ...
        case 'theme_change':
            ...
            break;
        case 'toggle_preview':
            if(e.target.children.length != 2 || stoptimeout) {return;}

            stoptimeout = true;
            setTimeout(() => {
                stoptimeout = false;
            }, 500);
            for(const [key,elem] of Object.entries(e.target.children)) {
                getComputedStyle(elem).top == '0px'?elem.style.top = '100%':elem.style.top = '0px';
            }
            break;
    }
});
...
function ChangePreviewImage(input, index) {
    let card = input.parentElement.parentElement;

    let firstimg = card.querySelector('img:first-child');
    getComputedStyle(firstimg).top == '0px'?firstimg.style.top = '100%':firstimg.style.top = '0px';
    
    let secondimg = card.querySelector('img:last-child');
    getComputedStyle(secondimg).top == '0px'?secondimg.style.top = '100%':secondimg.style.top = '0px';
}
